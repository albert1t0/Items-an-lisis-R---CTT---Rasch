...
```{r}
str.ciencias <- import("./Estructuras/00918_Versión_ciencias_COMPLETO.xls", sheet = 1, skip = 2, col_names = TRUE)

str.letras <- import("./Estructuras/00918_Versión_letras_COMPLETO.xls", sheet = 1, skip = 2, col_names = TRUE)
...
temp.diff <- c(6,8,10,12) # Se retiran de anclaje 19,32,37,44

diff.ciencias.mat <- mean(dstr.ciencias.mat[-temp.diff]) - 
  mean(-rciencias.mat$betapar[1:48][indicec_mat][-temp.diff])
...
diff.ciencias.red <- mean(dstr.ciencias.red) - mean(-rciencias.red$betapar)
...
temp.diff <- 7  # Se retiran 23
diff.ciencias.lec <- mean(dstr.ciencias.lec[-temp.diff]) - 
  mean(-rciencias.lec$betapar[-temp.diff])
...
diff.letras.lec <- mean(dstr.letras.lec) - mean(-rletras.lec$betapar)
...
temp.diff <- c(6,9) # Se retira el 16 y 25
diff.letras.red <- mean(dstr.letras.red[-temp.diff]) - 
  mean(-rletras.red$betapar[-temp.diff])
...
temp.diff <- c(4,9,10) # Se retira 13, 34 y 36
diff.letras.mat <- mean(dstr.letras.mat[-temp.diff]) - 
  mean(-rletras.mat$betapar[-temp.diff])

```

```{r}
estructura.ciencias <- data.frame(
  medicion = c(-rciencias.mat$betapar + diff.ciencias.mat + mean(dstr.ciencias[1:48][indicec_mat]),
               -rciencias.red$betapar + diff.ciencias.red + mean(dstr.ciencias[49:72][indicec_red]),
               -rciencias.lec$betapar + diff.ciencias.lec + mean(dstr.ciencias[73:96][indicec_lec])),
...
estructura.letras <- data.frame(
  medicion = c(-rletras.lec$betapar + diff.letras.lec + mean(dstr.letras[1:28][indicel_lec]),
               -rletras.red$betapar + diff.letras.red + mean(dstr.letras[29:56][indicel_red]),
               -rletras.mat$betapar + diff.letras.mat + mean(dstr.letras[57:96][indicel_mat])),
...
```{r}
write.csv(estructura.ciencias1, "./Estructura_ciencias1B.csv", row.names = FALSE)
write.csv(estructura.ciencias2, "./Estructura_ciencias2B.csv", row.names = FALSE)
write.csv(estructura.letras1, "./Estructura_letras1B.csv", row.names = FALSE)
write.csv(estructura.letras2, "./Estructura_letras2B.csv", row.names = FALSE)
```

```{r}

# En este bloque se arma la prueba con puntajes parciales para valida la posibilidad
# de utilziar puntajes brutos para el concurso de admisión.

# Versión 1 ciencias

v1ciencias <- bind_cols(ciencias1$code,
                        ciencias1.mat$score,
                        ciencias1.red$score,
                        ciencias1.lec$score,
                        pciencias1.mat[["theta.table"]][["Person Parameter"]],
                        pciencias1.red[["theta.table"]][["Person Parameter"]],
                        pciencias1.lec[["theta.table"]][["Person Parameter"]]) 
names(v1ciencias) <- c("code", "mat", "red", "lec", "smat", "sred", "slec")
